    ...
    for event in pygame.event.get():              
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_s:
                ...
                while CONT:
                    CONT = False
                    print("tyring to solve")
                    for i in range(GRID_HEIGHT):
                        for j in range(GRID_WIDTH):
                            if validate_grid[i][j].state not in [11, 10, 0]: #not a mine or not in game
                                num = validate_grid[i][j].state
                                empty = []
                                for x in [-1, 0, 1]:
                                    for y in [-1, 0, 1]:
                                        if not (x == y == 0):
                                            try:
                                                t = validate_grid[i+x][j+y].state
                                            except:
                                                t = 'N'
                                            if  t == 10 or t == 11: # mine
                                                num -= 1
                                            elif t == 9 and not validate_grid[i+x][j+y].safe:
                                                empty.append((i+x, j+y))
                                if num == len(empty) and num != 0 :
                                    for this_one in empty:
                                        validate_grid[this_one[0]][this_one[1]].state = 10
                                        validate_grid[this_one[0]][this_one[1]].update_texture()
                                        print (f"checked {empty} as mine")
                                    CONT = True
                                if num == 0 and len(empty)>0:
                                    CONT = True
                                    for this_one in empty:
                                        validate_grid[this_one[0]][this_one[1]].safe = True
                                        print (f"checked {empty} as safe")           
                print('done solving')                     
